...
# first extension adds grain size as a predictor of case-level varying 
# intercepts (omega)
# adapted from: 
...
# case-level sd as a function of Extent, adjusting for grain size
peng_sd_extent <- brm(bf(z | se(sqrt(var_z)) ~ ln_Grain + 
                           (1 | Study) +
                           (1 | gr(Study:Case, by = Extent))),
                      prior = c(prior(normal(0.3,1), class = Intercept),
                                prior(normal(0,1), class = b),
                                prior(normal(0,1), class = sd)),
                      data = dat,
...
peng_sd_extent_kfold <- kfold(peng_sd_extent,
                              folds = 'stratified',
                              group = 'Study',
                              k = 10)

save(peng_sd_extent, peng_sd_extent_kfold,
     file = paste0(wkdir, 'native-exotic-richness-relationships/model-fits-CV-results/peng-m3.Rdata')
     
# sigma (i.e., parameter associated with scale) as a function of grain, 
peng_sigma_linear <- brm(bf(z | se(sqrt(var_z), sigma = TRUE) ~ ln_Grain + (1 | Study / Case),
             sigma ~ ln_Grain),
          prior = c(prior(normal(0.3,1), class = Intercept),
                    prior(normal(0,1), class = b),
...
# varying study level residuals (estimated with "true" scale parameter: sigma)
peng_sigma_study <- brm(bf(z | se(sqrt(var_z), sigma = TRUE) ~ ln_Grain + (1 | Study / Case),
             sigma ~ 1 + (1 | Study)),
          prior = c(prior(normal(0.3,1), class = Intercept),
                    prior(normal(0,1), class = b),
                    prior(normal(0,1), class = Intercept, dpar = sigma),
...
save(peng_sigma_study, peng_sigma_study_kfold,
     file = paste0(wkdir, 'native-exotic-richness-relationships/model-fits-CV-results/peng-m5.Rdata')