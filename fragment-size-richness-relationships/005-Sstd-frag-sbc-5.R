...
# priors define the data generating process
frag_priors5 <- prior(normal(2.5,0.1), class = Intercept) +
  ...
  prior(normal(0.45,0.05), class = sd, dpar = sigma, coef = Intercept, group = dataset_label) +
  prior(normal(0.05,0.05), class = sd, dpar = sigma, coef = c.lfs, group = dataset_label) 

frag_generator5 <- SBC::SBC_generator_brms(bf(S_std_mean ~ c.lfs + (c.lfs | rho | dataset_label),
                                              sigma ~ c.lfs + (c.lfs | rho | dataset_label)),
                                           family = lognormal(),
...
label5 <- as_labeller(c("b_Intercept" = "beta[0]",
                        ...
                        "cor_dataset_label__c.lfs__sigma_c.lfs" = "rho[sigma[1]~sigma[1]^sigma]"), 
                      default = label_parsed)


pdf(paste0(wkdir, 'fragment-size-richness-relationships/figures/frag_sbc5.pdf'),
    width = 7, height = 5)

plot_rank_hist(frag_results5[ok_frag5],
               variables = c("b_c.lfs",
                             "b_sigma_Intercept",
                             "b_sigma_c.lfs",
                             ...
                             "cor_dataset_label__c.lfs__sigma_c.lfs")) +
...
plot_coverage(frag_results5[ok_frag5],
              variables = c("b_c.lfs",
                            "b_sigma_Intercept",
                            "b_sigma_c.lfs",
                            "sd_dataset_label__c.lfs", 
...
plot_coverage_diff(frag_results5[ok_frag5],
              variables = c("b_c.lfs",
                            "b_sigma_Intercept",
                            "b_sigma_c.lfs",
                            "sd_dataset_label__c.lfs", 
...
plot_ecdf(frag_results5[ok_frag5],
          variables = c("b_c.lfs",
                        "b_sigma_Intercept",
                        "b_sigma_c.lfs",
                        "sd_dataset_label__c.lfs", 
...
plot_ecdf_diff(frag_results5[ok_frag5],
          variables = c("b_c.lfs",
                        "b_sigma_Intercept",
                        "b_sigma_c.lfs",
                        "sd_dataset_label__c.lfs", 
...
plot_sim_estimated(frag_results5[ok_frag5],
                   variables = c("b_c.lfs",
                                 "b_sigma_Intercept",
                                 "b_sigma_c.lfs",
                                 "sd_dataset_label__c.lfs", 
  ...
  facet_wrap(~factor(variable, 
                     c("b_Intercept", "b_c.lfs",
                       ...
                       "sd_dataset_label__sigma_c.lfs",
                       "cor_dataset_label__c.lfs__sigma_c.lfs")),
             labeller = label5, scales = 'free') 
dev.off()

# save 
print(object.size(frag_results5[ok_frag5]), units = "Gb")

frag_sbc_results_5 <- frag_results5[ok_frag5]
